...
const qSchema = z.object({
  category: z.enum(['moviles', 'covers', 'accesorios']).optional(),
...
export async function GET(req: NextRequest) {
  ...
  const q = qSchema.parse({
    ...
    take: url.searchParams.get('take') || '12'
  });

  // ⬇️ Tipado correcto para @vercel/kv: T debe ser un array -> string[]
  const ids =
    (q.category
      ? await kv.smembers<string[]>(`products:cat:${q.category}`)
      : await kv.smembers<string[]>('products:index')) ?? [];
...
export async function POST(req: NextRequest) {
  ...
  await kv.sadd(`products:cat:${category}`, id);
